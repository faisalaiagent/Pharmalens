...
type Props = {
  ...
  onLiveDataChange?: (info: { brandCount: number; genericCount: number }) => void;
};
...
export function LookupApp({ records: baseRecords, onLiveDataChange }: Props) {
  // Merge admin-added overrides from the shared database (via API) into the
  // live dataset. This replaces the old localStorage-based approach, which
  // caused different browsers/devices to see different "live" data — admin
  // additions are now stored centrally and visible to every visitor.
  ...
  useEffect(() => {
    ...
    const reportCount = (all: DrugBrandRecord[]) => {
      onLiveDataChange?.({
        brandCount: all.length,
        genericCount: new Set(all.map((r) => r.genericSlug)).size,
      });
    };

    fetch("/api/overrides")
      .then(res => (res.ok ? res.json() : { overrides: [] }))
      .then((data: { overrides: Array<{
        id: string; genericName: string; brandName: string; strength: string;
        dosageForm: string; manufacturer: string; note?: string | null; addedAt?: string;
      }> }) => {
        const overrides = data.overrides ?? [];
        if (overrides.length === 0) {
          reportCount(baseRecords);
          return;
        }
        const adminRecords = overrides.map(o => ({
          id: o.id,
          genericName: o.genericName,
          genericSlug: slugify(o.genericName),
          brandName: o.brandName,
          activeIngredient: o.genericName,
          strength: o.strength,
          dosageForm: o.dosageForm,
          manufacturer: o.manufacturer,
          country: "Pakistan",
          sourceName: "Admin Added",
          sourceUrl: undefined,
          verificationStatus: "pending_review" as const,
          lastVerifiedAt: o.addedAt ?? new Date().toISOString(),
          sourceNote: o.note ?? "Added via Admin Panel",
        }));
        const merged = [...baseRecords, ...adminRecords];
        setRecords(merged);
        reportCount(merged);
      })
      .catch(() => {
        // Fall back silently to base dataset if the API is unreachable
        // (e.g. no DATABASE_URL configured in this environment).
        reportCount(baseRecords);
      });
  }, [baseRecords, onLiveDataChange]);
      ...
      <section className="space-y-5 min-w-0" aria-live="polite">
        ...
        {results.length ? (
              ...
              {pagedResults.map((record) => (
                ...
                >
                  ...
                  <dl className="mt-3 grid grid-cols-2 gap-3 text-sm">
                    ...
                    <MobileFact label="Verified" value={formatVerifiedDate(record.lastVerifiedAt)} />
                  </dl>
                </article>
              ))}
            </div>
          ...
          {totalPages > 1 && (() => {
            ...
            // This keeps the control compact and wrapping cleanly even with 100+ pages.
            const windowSize = 2;
            const pageNumbers: (number | "ellipsis")[] = [];
            const addPage = (p: number) => pageNumbers.push(p);

            addPage(1);
            if (page - windowSize > 2) pageNumbers.push("ellipsis");
            for (let p = Math.max(2, page - windowSize); p <= Math.min(totalPages - 1, page + windowSize); p++) {
              addPage(p);
            }
            if (page + windowSize < totalPages - 1) pageNumbers.push("ellipsis");
              ...
              >
                ...
                <div className="flex flex-wrap items-center justify-center gap-1.5">
                  <button
                    ...
                    className="rounded-xl px-3 py-2 text-sm font-medium transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed"
                  ...
                  {pageNumbers.map((p, i) =>
                    p === "ellipsis" ? (
                      <span key={`e-${i}`} className="px-1 text-sm" style={{ color: "hsl(215 20% 40%)" }}>
                        …
                      </span>
                    ) : (
                      <button
                ...
                {totalPages > 10 && (
                  <div className="flex items-center gap-2 text-sm" style={{ color: "hsl(215 20% 45%)" }}>
                    ...
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          const val = Math.min(totalPages, Math.max(1, Number((e.target as HTMLInputElement).value) || 1));
                          setPage(val);
...
function formatVerifiedDate(value: string): string {
  // Dataset records already store "YYYY-MM-DD"; admin-added overrides store
  // a full ISO timestamp (toISOString()). Normalize both to the same clean
  // date-only format so the Verification column never looks inconsistent.
  ...
  return (
    <div className="space-y-1.5">
      ...
      <p className="text-xs" style={{ color: "hsl(215 20% 45%)" }}>
        {formatVerifiedDate(record.lastVerifiedAt)}
      </p>
      {record.sourceUrl ? (
        <a

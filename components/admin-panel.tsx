...
const STORAGE_KEY = "pharmalens_admin_overrides";
const LOG_KEY = "pharmalens_admin_log";
...
export default function AdminPanel() {
  ...
  const persist = useCallback((records: BrandRecord[], logs: AdminLog[]) => {
  ...
  function addBrand() {
    ...
    const newOverrides = [rec, ...overrides];
    const newLog = [entry, ...log];
    setOverrides(newOverrides);
    setLog(newLog);
    persist(newOverrides, newLog);
    setForm({ generic: "", brand: "", strength: "", dosageForm: "", manufacturer: "", note: "" });
    setFormErrors({});
    showToast(`✓ "${rec.brand}" added`);
    setTab("overrides");
  }
  ...
  function confirmRemove() {
    ...
    const newOverrides = overrides.map(r =>
      r.id === removeTarget.id
    ...
    const entry: AdminLog = {
      id: genId(),
      action: "remove",
      brand: removeTarget.brand,
      generic: removeTarget.generic,
      manufacturer: removeTarget.manufacturer,
      reason: removeReason || "No reason given",
  ...
  function restoreBrand(id: string) {
    const target = overrides.find(r => r.id === id);
    if (!target) return;
    const newOverrides = overrides.map(r =>
      r.id === id ? { ...r, status: "active" as const, removedAt: undefined } : r
    );
    const entry: AdminLog = {
      id: genId(),
      action: "restore",
  ...
  function permanentDelete(id: string) {
    const target = overrides.find(r => r.id === id);
    if (!target) return;
    const newOverrides = overrides.filter(r => r.id !== id);
    const entry: AdminLog = {
      id: genId(),
      action: "remove",
      brand: target.brand,
  ...
  function saveEdit() {
    ...
    const newOverrides = overrides.map(r =>
      r.id === editTarget.id
        ? { ...r, generic: editForm.generic, brand: editForm.brand, strength: editForm.strength, form: editForm.dosageForm, manufacturer: editForm.manufacturer, note: editForm.note }
        : r
    );
    const entry: AdminLog = {
      id: genId(),
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#070c14", fontFamily: "system-ui, sans-serif", color: "#e2e8f0" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 24px" }}>
        ...
        {/* Info banner */}
        <div style={{ background: "rgba(251,191,36,0.08)", border: "1px solid rgba(251,191,36,0.2)", borderRadius: 12, padding: "12px 16px", marginBottom: 24, fontSize: 13, color: "#fbbf24" }}>
          ⚠️ This panel manages <strong>admin overrides</strong> stored in your browser. These additions/removals are visible only in this browser session. To permanently update the dataset for all users, export and apply changes to <code>sample-data.ts</code>.
        </div>
          ...
          <div>
            {log.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                <button onClick={() => { if (confirm("Clear all log entries?")) { setLog([]); localStorage.setItem(LOG_KEY, "[]"); } }} style={{ ...css.btn("ghost"), marginTop: 8, alignSelf: "flex-start" }}>
                  Clear Log
                </button>
              </div>
            )}
